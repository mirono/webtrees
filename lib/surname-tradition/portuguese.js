...
// Port of app/SurnameTradition/PortugueseSurnameTradition.php.
// See docs/php-to-js-migration/task-05-surname-tradition-subclasses.md.
//
// newParentNames()'s PHP source is a `switch ($sex) { case 'M': ...; case
// 'F': ...; }` with no default — for any sex other than 'M'/'F', or when
// REGEX_SURNS doesn't match at all, execution falls through to the
// post-switch `return [buildName('// //', ...)]`. Ported below as
// sequential ifs ending in the same fallback, preserving that exact
// fallthrough shape rather than assuming sex is always 'M' or 'F'.

import { DefaultSurnameTradition, REGEX_SURNS } from './default.js';
import { VALUE_BIRTH } from './name-type.js';

export class PortugueseSurnameTradition extends DefaultSurnameTradition {
  /**
   * The name of this surname tradition.
   */
  name() {
    return this.i18n.translateContext('Surname tradition', 'Portuguese');
  }
   ...
   */
  description() {
    return this.i18n.translate('Children take one surname from the mother and one surname from the father.');
  }
  ...
  newChildNames(father, mother, sex) {
    ...
    const fatherSurname = fatherMatch ? fatherMatch.groups.SURN2 : '';

    const motherMatch = this.extractName(mother).match(REGEX_SURNS);
    const motherSurname = motherMatch ? motherMatch.groups.SURN2 : '';
