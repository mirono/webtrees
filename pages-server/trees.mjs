...
export async function accessibleTrees(pool, { userId, isAdmin }) {
  if (isAdmin) {
    const result = await pool.query(
      'SELECT gedcom_id, gedcom_name, imported, private FROM wt_gedcom WHERE gedcom_id > 0 ORDER BY sort_order, title',
    );
  ...
  const result = await pool.query(
    `SELECT gedcom.gedcom_id, gedcom.gedcom_name, gedcom.imported, gedcom.private
